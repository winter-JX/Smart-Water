...
void EXTI15_10_IRQHandler(void)//中断函数的格式
{
	...
	if (EXTI_GetITStatus(EXTI_Line15)==SET)//判断是否为EXTI15进来，返回值为 SET or RESET
	{
		LED1_Turn();
		num_jing += 1;
		num_jing = num_jing % 8;
		...
		EXTI_ClearITPendingBit(EXTI_Line15);//清除中断函数的标志位，否则程序将一直卡在中断函数中
	}
...
void TIM3_IRQHandler(void) //TIM3 中断
{
    if (TIM_GetITStatus(TIM3, TIM_IT_Update) != RESET) //检查 TIM3 更新中断发生与否
    {
				...
				if(flag_init == 0)
				{
					...
					if(Serial_RxFlag2 == 1){
						shuiWei = 0;
						for(i_shuiwei=0;i_shuiwei<4;i_shuiwei++){
							shuiWei *= 10;
							tmp_num = Serial_RxPacket2[i_shuiwei] - '0';
							shuiWei += tmp_num; 
						}
						
						OLED_ShowNum(40,0,shuiWei,4,16,1);
					
						//OLED_ShowString(40,0,Serial_RxPacket2,16,1);
						Serial_RxFlag2 = 0;
					}
				}
        ...
        golbal_flow.receive_flag = 1;
				OLED_ShowNum(40,15,golbal_flow.instant,3,16,1);
